...
(function (exports) {
    ...
    exports.initialize = function (defaultEventHandlers, manageMode) {
      ...
      //  $('#select-timezone').val(Intl.DateTimeFormat().resolvedOptions().timeZone);
         ...
         $('#select-timezone').val('America/Buenos_Aires');
    ...
    function bindEventHandlers() {
        ...
        /*Evento: Botón "Siguiente paso" "Clicked"
         *
         * Este controlador se activa cada vez que el usuario presiona el botón "siguiente" en el asistente de libros.
         * Es posible que se realicen algunas tareas especiales, según el paso del asistente actual.*/

        $('.button-next').on('click', function () 
        {
            //Cambio estilo nuevo steps Fabio
            if($('#step-1').hasClass('active-step') && $('#select-provider').val().length > 0){
               
                $('#li-2').removeClass('gris-inactivo');
                $('#li-2').addClass('amarillo');
                $('#st-2').removeClass('gris-inactivo');
                $('#st-2').addClass('amarillo');
            }
            if($('#step-2').hasClass('active-step')){
               
                $('#li-3').removeClass('gris-inactivo');
                $('#li-3').addClass('naranja');
                $('#st-3').removeClass('gris-inactivo');
                $('#st-3').addClass('naranja');
            }
            if($('#step-3').hasClass('active-step') && $('#dni').val().length > 0 && $('#sexo').val().length > 0 && $('#email').val().length > 0 && $('#phone-number').val().length > 0){
               
                $('#li-4').removeClass('gris-inactivo');
                $('#li-4').addClass('gris');
                $('#st-4').removeClass('gris-inactivo');
                $('#st-4').addClass('gris');
            }
            //Cambio estilo nuevo steps Fabio
            ...
            if ($(this).attr('data-step_index') === '2') 
            {
                ...
                if ( $('#notes').val() == 'CIDI')
                {
                //alert('ver paso 3 TOMOR LOS VALORES DESDE EL TOKEN');
                ...
                var nextTabIndex = 4;//parseInt($(this).attr('data-step_index')) + 2;
        ...
        $('.button-back').on('click', function () 
        {
            ...
            if($('#step-2').hasClass('active-step')){
                ...
                $('#li-2').removeClass('amarillo');
                $('#li-2').addClass('gris-inactivo');
                $('#st-2').removeClass('amarillo');
                $('#st-2').addClass('gris-inactivo');
            }
            if($('#step-3').hasClass('active-step')){
               
                $('#li-3').removeClass('naranja');
                $('#li-3').addClass('gris-inactivo');
                $('#st-3').removeClass('naranja');
                $('#st-3').addClass('gris-inactivo');
            }
            if($('#step-4').hasClass('active-step')){
               
                $('#li-4').removeClass('gris');
                $('#li-4').addClass('gris-inactivo');
                $('#st-4').removeClass('gris');
                $('#st-4').addClass('gris-inactivo');
            }
            //Cambio estilo nuevo steps Fabio
            ...
            //alert(prevTabIndex);
            // si estamos en CIDI
            if ( (prevTabIndex==3) && ($('#notes').val() == 'CIDI'))
            {
                $(this).parents().eq(1).hide('fade', function () {
                            $('.active-step').removeClass('active-step');
                            $('#step-' + 2).addClass('active-step');
                            $('#wizard-frame-' + 2).show('fade');
                        });
         ...
         *
         * Evento: Formulario de reserva de cita "Enviar" Antes de enviar el formulario al servidor, debemos asegurarnos de que
         * Mientras tanto, la fecha / hora de la cita seleccionada no fue reservada por
         * otro cliente o evento.
         * 
         * 
         * 
         * @param {jQuery.Event} event
     ...
     * Cada vez que se ejecuta esta función, actualiza la página de confirmación con la última configuración del cliente y la entrada para la reserva de la cita.
     */
    exports.updateConfirmFrame = function ()
    {
        ...
        //Cargo en una variable los datos del tramite seleccionado para obtener la duracion del mismo
        var serviceSelected = GlobalVariables.availableServices.find(function (availableService) {
            ...
            'html': [
                ...
                $('<p/>', {
                    'html': [
                        ...
                        $('<br/>'),
                        $('<span/>', {
                            'text': 'Hora: ' + ' ' + $('.selected-hour').text()
                        }),
                        $('<br/>'),
                        $('<span/>', {
                            'text': 'Duración del trámite: ' + serviceSelected.duration + ' ' + EALang.minutes
                        }),
                        $('<br/>'),
                        $('<span/>', {
        ...
        // Customer Details
        var sexo = GeneralFunctions.escapeHtml($('#sexo').val());
        var dni = GeneralFunctions.escapeHtml($('#dni').val());
        ...
        data.customer = {
            ...
            timezone:'America/Buenos_Aires'//$('#select-timezone').val()
        };
     ...
     * 
     * Este método aplica los datos de la cita al asistente para
     * que el usuario puede comenzar a realizar cambios en un registro existente
     * 
     * 
     */
    function applyAppointmentData(appointment, provider, customer) {
        try {
            ...
            // Apply Customer's Data
             $('#dni').val(customer.dni);
             $('#sexo').val(customer.sexo);
            ...
            if (customer.timezone) {
                $('#select-timezone').val('America/Buenos_Aires') // antes customer.timezone
            }
     ...
     * 
     * Este método actualiza el contenido html de un div con una breve descripción del
     * servicio seleccionado por el usuario (solo si está disponible en db). Esto es útil para
     * clientes al seleccionar el servicio correcto.
     * 
     * 
     */
    function updateServiceDescription(serviceId) {
        ...
        if (service.duration) {
            $('<h6/>', {
                'text': 'Duración: ' + service.duration + ' ' + EALang.minutes
            })
                .appendTo($serviceDescription);
        }
